...
### モバイル（2026-09-16 /design-review で追加）
- **表は縦積みにする。横スクロールで結論（合計）を隠さない。** `OnDiagram.tsx` の2レイアウト方式に倣う
- **ナビは消さない。** `hidden md:flex` で隠すだけにせず、`md` 未満は横スクロールのピル行を出す（ハンバーガーは様式に合わない）
- **ピルは1行で収まる文言にだけ使う。** 折れる可能性がある本文は 20px 角丸（カードと同じ）にする
- **リボンは見出しにだけ掛ける。** 本文・凡例・CTAの背後に来たら位置を上へずらすか、本文を白地に載せる
- **選択状態（言語トグル等）は黒塗り。** 青は装飾専用（ルール3の適用範囲）
- 文字サイズはワークスペース規定が優先（`font-pixel` 最小 0.7rem・和文最小 `text-sm`）。
  SVG 内の `fontSize` は viewBox 倍率で拡大されるので、**描画後のサイズ**で判定する
